...
class SearchBar extends React.Component<SearchBarProps, SelectBarState> {
  ...
  render() {
    ...
    const listBed = [getTranslation(this.props.lang, 'Bedrooms'), '1', '2', '3', '4'];
    const listBath = [getTranslation(this.props.lang, 'Bathrooms'), '1', '2', '3', '4'];
    return (
        ...
        <form className='form-inline' role='form'>
          ...
          <div className={`form-group${this.state.isAdvance ? ' adv' : ' hidden-xs'}`}>
            <div className='checkbox custom-checkbox'>
              <label>
                <input type='checkbox' />
                <Icon name='check' /> {getTranslation(this.props.lang, 'For rent')}
              </label>
            </div>
          </div>
          <div className={`form-group${!this.state.isAdvance ? ' hidden-xs' : ''}`}>
            <div className='checkbox custom-checkbox'>
              <label>
                <input type='checkbox' />
                <Icon name='check' /> {getTranslation(this.props.lang, 'For sale')}{' '}
              </label>
            </div>
          </div>
          <div className='form-group'>
            <Link to='/search' className='btn btn-green isThemeBtn'>
              {getTranslation(this.props.lang, 'Search')}
            </Link>
            <a
              href='#'
              className={`btn btn-o btn-white pull-right visible-xs${
                this.state.isAdvance ? ' advBtnActive' : ''
              }`}
